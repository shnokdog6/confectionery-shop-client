...
export const ProductList = memo(() => {
    ...
    const {data, isLoading, isError, error} = useProductsQuery({category: category?.id}, {
        //skip: category == null
    });
    ...
    return (
        <Container>
            ...
            <StyledRow>
                ...
                {isError && <div className="w-auto">{JSON.stringify(error)} - Не удалось загрузить товар</div>}

                {
                    data?.map((item) => (
                        <Col xl={3} lg={4} md={6}>
                            <ProductCard key={`${item.id}.${item.name}`} src={item}/>
                        </Col>)
                    )}
            </StyledRow>
        </Container>
